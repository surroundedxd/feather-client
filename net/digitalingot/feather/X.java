...
public class x
{
    ...
    public x() {
        ...
        this.ay = new A();
        net.digitalingot.feather.ao.bH.a((c, p1) -> {
            if (c == net.digitalingot.feather.ao.c.END) {
                this.al();
            }
        });
    }
    ...
    private void aj() {
        ...
        x.as.execute(() -> {
            try {
                x.ar.func_110536_a(x.aq);
                if (resource != null) {
                    resource.func_110527_b();
                    try {
                ...
                kr.Gh.error("Error while loading cloak model", (Throwable)ex);
            }
            finally {
                this.at = false;
            }
    ...
    private void m(@NotNull final String s) {
        ...
        x.as.execute(() -> {
            try {
                ...
                if (!(!file.exists())) {
                    new G(x.ap).a(new WebPImage(jz.m(file).tt().tu())).aB();
                    if (f == null) {
                        kr.Gh.error("[Cloak] Failed to load animation for " + s);
                    }
                    else {
                        this.ax.put(s, f);
                    }
    ...
    private p d(final UUID uuid) {
        final gs hm = x.ao.fb().hM();
        if (hm == null) {
            return null;
        }
        final List<p> list = hm.ib().hT().get(uuid);
        if (list == null) {
            return null;
        }
        for (final p p : list) {
    ...
    static {
        ...
        ar = Minecraft.func_71410_x().func_110442_L();
